...
from odor_topology.robustness import build_sample_index_runs, run_landmark_metric_analysis


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Compare H1 robustness across POM and chemical baseline representations "
            "using greedy farthest-point landmarks on within-run distance matrices."
        )
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=1000,
        help="Points per repeated subsample run before landmark selection",
    )
    parser.add_argument(
        "--landmark-count",
        type=int,
        default=250,
        help="Number of greedy farthest-point landmarks retained per run",
    )
    parser.add_argument(
        "--runs",
        type=int,
...
def main() -> None:
    ...
    report: dict[str, object] = {
        ...
        "settings": {
            ...
            "comparison_note": (
                "This analysis uses greedy farthest-point landmarks on within-run distance matrices. "
    ...
    for representation in representations:
        ...
        for metric_spec in representation["metrics"]:
            ...
            result = run_landmark_metric_analysis(
                ...
                base_seed=config.random_seed,
                n_landmarks=args.landmark_count,
            )
    ...
    report_path = (
        ...
        / "outputs"
        / "reports"
        / f"{report_prefix}_representation_robustness_comparison_landmark.json"
    )
    summary_path = (
        PROJECT_ROOT
        / "outputs"
        / "reports"
        / f"{report_prefix}_representation_robustness_summary_landmark.csv"
    )
